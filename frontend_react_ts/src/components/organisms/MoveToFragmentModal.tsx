...
export const MoveToFragmentModal: React.FC<MoveToFragmentModalProps> = ({
  ...
  const handleConfirmMove = async () => {
    ...
    const targetFragment = availableTargets.find(
      (f) => f.id === selectedTargetId
    );
    ...
    try {
      ...
      const userEntries = (sourceDetail.entries || []).filter(
        (e) => e.role === 'user'
      );
      if (userEntries.length > 0) {
        for (const entry of userEntries) {
          await observationsApi.createEntry(selectedTargetId, {
      ...
      await queryClient.invalidateQueries({
        queryKey: queryKeys.fragments.detail(selectedTargetId),
      });

      toast(
        `Pemikiran berhasil dipindahkan ke: "${targetTitle.slice(0, 30)}..."`,
        'success'
      );
      onClose();
      ...
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="py-4 space-y-3 flex-1 overflow-y-auto scrollbar-thin pr-1">
          <p className="text-xs text-zinc-400 font-sans">
            Pilih fragment tujuan untuk memindahkan pemikiran dan merajut benang
            merah baru:
          </p>
