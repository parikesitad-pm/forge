...
export function useAddThought(fragmentId: number | string) {
  ...
  return useMutation({
    mutationFn: (content: string) =>
      observationsApi.createEntry(fragmentId, { content }),
    onSuccess: (newEntry) => {
      queryClient.setQueryData<FragmentDetail>(
        queryKeys.fragments.detail(fragmentId),
        (old) => {
          if (!old) return old;
          return {
            ...old,
...
export function useToggleSpark(fragmentId: number | string) {
  ...
  return useMutation({
    mutationFn: ({
      ...
      currentlyPinned
        ? sparksApi.unpin(observationId)
        : sparksApi.pin(observationId),

    // Optimistic Update
    onMutate: async ({ observationId, currentlyPinned }) => {
      await queryClient.cancelQueries({
        queryKey: queryKeys.fragments.detail(fragmentId),
      });
      const previousFragment = queryClient.getQueryData<FragmentDetail>(
        queryKeys.fragments.detail(fragmentId)
      );
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.sparks.byFragment(fragmentId),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.fragments.all });
    },
  });
